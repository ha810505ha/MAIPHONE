...
export default function useChatroomImportExport({ currentCharacter, characters, chatHistory, chatModes, chatBackgrounds, chatLorebookBindings, innerThoughtSettings, setChatHistory, setChatModes, setChatBackgrounds, setChatLorebookBindings, setInnerThoughtSettings, resetOpenChat, normalizeBackground, downloadJsonFile, showToast, sanitizeText, tr }) {
  ...
  const deleteChatroom = useCallback((characterId, characterName = "這個角色") => {
    ...
    [setChatHistory, setChatModes, setChatLorebookBindings, setChatBackgrounds, setInnerThoughtSettings].forEach((setter) => removeKey(setter, characterId));
    if (currentCharacter?.id === characterId) resetOpenChat();
    showToast(tr("聊天室已刪除", "Chatroom deleted", "チャットルームを削除しました", "채팅방을 삭제했습니다"));
  }, [currentCharacter, resetOpenChat, showToast, tr, setChatHistory, setChatModes, setChatLorebookBindings, setChatBackgrounds, setInnerThoughtSettings]);

  const exportChatroom = useCallback((characterId, characterName = "這個角色") => {
    if (!characterId) return;
    const payload = { format: "maliphone-chatroom", formatVersion: 1, exportedAt: new Date().toISOString(), characterId, characterName, chatHistory: chatHistory?.[characterId] || [], chatMode: chatModes?.[characterId] || "online", chatBackground: chatBackgrounds?.[characterId] || "", chatLorebookBinding: chatLorebookBindings?.[characterId] || null, innerThoughtSetting: innerThoughtSettings?.[characterId] || null };
  ...
  }, [chatHistory, chatModes, chatBackgrounds, chatLorebookBindings, innerThoughtSettings, sanitizeText, downloadJsonFile, showToast, tr]);
  ...
  }, [preview, targetId, tr, setChatHistory, setChatModes, setChatBackgrounds, normalizeBackground, setChatLorebookBindings, setInnerThoughtSettings, currentCharacter, resetOpenChat, characters, showToast]);
