...
import { MILESTONES, companionDays, bondTier, milestoneForLocale } from "./petDiary";
import { legacyGenderName, legacyPersonalityName } from "./petProfile.js";

const aiReady = (apiConfig) => isAiConfigReady(apiConfig);
const languageName = (locale) => ({ "zh-TW": "臺灣繁體中文", "zh-CN": "中国大陆简体中文", en: "English", ja: "日本語", ko: "한국어" })[locale] || "臺灣繁體中文";

const personaOf = (data, locale = "zh-TW") => {
  const profile = data.petProfile || {};
  const name = profile.name?.trim() || "麻糬";
  let persona = `你是一隻寵物，名叫「${name}」，品種：${profile.species || "小狗"}`;
  const gender = legacyGenderName(profile.gender);
  if (gender !== "未設定") persona += `，性別：${gender}`;
  persona += `。\n主要個性：${legacyPersonalityName(profile.primaryPersonality)}`;
  ...
  persona += `\n你用第一人稱寫自己的日記，稱呼照顧你的人為主人。
規則：一律使用${languageName(locale)}、口吻符合個性、不要 emoji、不要 markdown、不要引號、內容具體，可自然呼應喜歡或討厭的事物。`;
  return persona;
};
...
export async function generateMilestoneTexts(keys, data, apiConfig, locale) {
...
時刻：
${items.map((key) => { const milestone = milestoneForLocale(key, locale); return `- ${key}：${milestone.title}（${milestone.hint}）`; }).join("\n")}
只輸出 JSON 物件，不要任何其他文字或 markdown 圍欄，結構：{${items.map((key) => `"${key}": "日記內容"`).join(", ")}}`;
  try {
    const raw = await callAI([{ role: "user", content: user }], { ...apiConfig, maxTokens: 4000 }, personaOf(data, locale), {
      app: "pet",
...
export async function generateLifeDiary(dayLog, data, apiConfig, locale) {
...
以這些真實發生的事為素材，寫一篇 50~90 字回顧昨天的日記，挑最有感覺的一兩件事寫，不要流水帳。只輸出日記內容本身。`;
  try {
    const text = stripQuotes(await callAI([{ role: "user", content: user }], { ...apiConfig, maxTokens: 4000 }, personaOf(data, locale), {
      app: "pet",
...
// 生日日記：公版先上，AI 成功再替換。
export async function generateBirthdayDiary(data, apiConfig, locale) {
...
export async function generateEntryReply(entry, data, apiConfig, locale) {
  if (!aiReady(apiConfig)) return null;
...
export async function generateNoteReply(entry, note, data, apiConfig, locale) {
  if (!aiReady(apiConfig)) return null;
