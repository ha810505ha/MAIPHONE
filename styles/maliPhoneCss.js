...
﻿const css = `
@import url('https://fonts.googleapis.com/css2?family=Noto+Serif+TC:wght@400;600;700&family=Quicksand:wght@300;400;500;600;700&family=Zen+Maru+Gothic:wght@300;400;500;700&display=swap');
...
.mp-icon-c{width:56px;height:56px;border-radius:18px;display:flex;align-items:center;justify-content:center;font-size:24px;background:rgba(255,255,255,.64);backdrop-filter:blur(14px);-webkit-backdrop-filter:blur(14px);border:1px solid rgba(255,255,255,.72);box-shadow:0 4px 14px rgba(0,0,0,.05);transition:box-shadow .2s;}
...
.mp-icon-c:hover{box-shadow:0 8px 18px rgba(244,143,177,.18);}
...
.mp-dock-i{width:56px;height:56px;border-radius:18px;display:flex;align-items:center;justify-content:center;font-size:24px;cursor:pointer;background:rgba(255,255,255,.68);border:1px solid rgba(255,255,255,.75);transition:transform .14s ease,background .15s,box-shadow .18s;touch-action:none;}
.mp-dock-i .mp-app-icon-img{border-radius:18px;}
.mp-dock-i:active{transform:scale(.95);}
.mp-dock-i:hover{background:rgba(255,255,255,.82);box-shadow:0 8px 16px rgba(255,255,255,.28);}
...
.mp-chat-pin{width:24px;height:24px;min-width:24px;border:none;border-radius:50%;display:flex;align-items:center;justify-content:center;background:transparent;color:#b07b8f;font-size:16px;cursor:pointer;transition:transform .12s,color .16s;}
.mp-chat-pin:hover{box-shadow:none;background:transparent;}
.mp-chat-pin:active{transform:scale(.9);}
.mp-chat-pin.active{background:transparent;color:#b85c7a;}
.mp-chat-switch{display:flex;gap:8px;align-items:center;margin:0 12px 6px;padding:5px;background:rgba(255,255,255,.58);border:1px solid rgba(244,143,177,.12);border-radius:16px;box-shadow:0 4px 14px rgba(244,143,177,.05);}
.mp-chat-switch-btn{position:relative;flex:1;border:none;background:transparent;color:#8e9dad;font-size:13px;font-weight:800;font-family:var(--mp-font);cursor:pointer;padding:8px 10px;border-radius:12px;transition:background .16s,color .16s,transform .16s,box-shadow .16s;}
.mp-chat-switch-btn.active{color:#b85c7a;background:linear-gradient(135deg,rgba(248,187,208,.42),rgba(252,228,236,.92));box-shadow:0 2px 8px rgba(244,143,177,.12);}
...
.mp-chat-row:hover{background:rgba(255,255,255,.58);}
.mp-chat-row:active{background:rgba(244,143,177,.08);}
.mp-chat-row.pinned{background:transparent;border-bottom-color:rgba(244,143,177,.14);}
.mp-chat-row.pinned:hover{background:rgba(255,255,255,.58);}
.mp-chat-row-avatar{width:46px;height:46px;border-radius:14px;flex-shrink:0;display:flex;align-items:center;justify-content:center;overflow:hidden;background:linear-gradient(135deg,var(--mp-pink-lt),var(--mp-blue-lt));font-size:20px;font-weight:800;color:#455a64;}
...
.mp-ci:hover{background:rgba(244,143,177,.05);border-bottom-color:rgba(244,143,177,.18);}
...
.mp-group-msg-avatar{width:36px;height:36px;border-radius:50%;overflow:hidden;display:flex;align-items:center;justify-content:center;background:linear-gradient(135deg,var(--mp-pink-lt),var(--mp-blue-lt));color:#455a64;font-size:15px;flex-shrink:0;}
...
.mp-retry-btn{margin-top:7px;padding:4px 10px;border-radius:999px;border:1px solid rgba(244,143,177,.24);background:rgba(244,143,177,.12);color:var(--mp-pink-dk);font-size:11px;font-weight:800;font-family:var(--mp-font);cursor:pointer;}
...
.mp-msg-img{max-width:100%;border-radius:10px;margin-top:4px;}
  ...
  @keyframes mpThoughtPulse{0%,100%{opacity:.45;transform:scale(1)}50%{opacity:1;transform:scale(1.24);text-shadow:0 0 7px rgba(224,111,158,.55)}}
  @media (prefers-reduced-motion:reduce){.mp-thought-unseen-icon{animation:none;opacity:1;text-shadow:0 0 5px rgba(224,111,158,.38)}}
  ...
  .mp-thought-content{margin:2px 0 3px 4px;padding:7px 10px;border-left:2px solid rgba(194,126,158,.42);background:rgba(255,255,255,.48);border-radius:0 6px 6px 0;font-size:11px;font-style:italic;line-height:1.55;color:var(--mp-txt);white-space:pre-wrap;word-break:break-word;animation:mpFi .16s;}
  ...
  .mp-switch.active{background:#e06f9e;}
  ...
  .mp-thought-record:hover{background:rgba(255,255,255,.28);}
  ...
  .mp-thought-history-pages button{width:26px;height:24px;padding:0;border:1px solid rgba(148,173,188,.3);border-radius:6px;background:rgba(255,255,255,.48);color:var(--mp-txt);display:inline-flex;align-items:center;justify-content:center;cursor:pointer;}
  ...
  @keyframes mpThoughtJump{0%,35%{filter:drop-shadow(0 0 7px rgba(224,111,158,.7));transform:translateX(0)}100%{filter:drop-shadow(0 0 0 rgba(224,111,158,0));transform:translateX(0)}}
  @media (prefers-reduced-motion:reduce){.mp-thought-jump-highlight{animation:none;filter:drop-shadow(0 0 5px rgba(224,111,158,.5));}}
...
.mp-voice-action:hover{color:var(--mp-pink-dk);background:rgba(244,143,177,.1);}
...
.mp-transfer-card{
  ...
  border-radius:22px;
  background:linear-gradient(180deg,rgba(255,255,255,.84),rgba(248,250,252,.9));
...
.mp-transfer-note{font-size:12px;line-height:1.5;color:var(--mp-txt-l);padding:8px 10px;background:rgba(255,255,255,.52);border:1px solid rgba(160,176,186,.14);border-radius:10px;position:relative;z-index:1;}
...
.mp-mode-tabs{display:grid;grid-template-columns:1fr 1fr;gap:6px;padding:3px;background:rgba(255,255,255,.55);border:1px solid rgba(231,197,214,.7);border-radius:12px;}
...
.mp-mode-tab.active{background:#fff;color:#6a1b9a;box-shadow:0 2px 8px rgba(106,27,154,.12);}
.mp-mode-hint{margin-top:7px;font-size:11px;line-height:1.5;color:var(--mp-txt-l);}
.mp-mode-sep{display:flex;align-items:center;gap:10px;margin:9px 0 7px;color:#6d4c41;font-size:11px;font-weight:800;letter-spacing:.5px;}
.mp-mode-sep::before,.mp-mode-sep::after{content:'';height:1px;flex:1;background:linear-gradient(90deg,rgba(121,85,72,0),rgba(121,85,72,.32));}
.mp-mode-sep::after{background:linear-gradient(90deg,rgba(121,85,72,.32),rgba(121,85,72,0));}
.mp-mode-sep span{padding:3px 10px;border-radius:999px;background:rgba(255,255,255,.72);border:1px solid rgba(121,85,72,.18);}
...
.mp-reality-msg{width:100%;max-width:100%;padding:8px 2px 10px;font-size:14px;line-height:1.9;color:#37474f;word-break:break-word;white-space:normal;}
...
.mp-reality-thought{color:#5e2387;font-family:'Noto Serif TC','PMingLiU','Songti TC',serif;font-style:italic;font-weight:600;}
.mp-reality-dialogue{color:#5d4037;font-family:var(--mp-font);font-weight:400;}
.mp-reality-strong{color:#ad3f68;font-family:var(--mp-font);font-weight:800;}
...
.mp-chat-actions{display:grid;grid-template-columns:repeat(4,1fr);gap:10px;padding:12px 14px;background:rgba(255,255,255,.72);border-top:1px solid rgba(236,205,220,.7);}
...
.mp-chat-action-i{width:44px;height:44px;border-radius:14px;display:flex;align-items:center;justify-content:center;background:#fff;border:1px solid rgba(231,197,214,.8);box-shadow:0 2px 8px rgba(0,0,0,.04);font-size:20px;}
.mp-inp-wrap{flex:1;min-width:0;display:flex;flex-direction:column;gap:3px;}
.mp-inp{width:100%;border:1px solid rgba(231,197,214,.9);outline:none;background:#fff;border-radius:18px;padding:10px 14px;font-size:13px;font-family:var(--mp-font);color:var(--mp-txt);resize:none;line-height:1.35;min-height:38px;max-height:86px;}
...
.mp-update-item{position:relative;padding:7px 9px 7px 22px;border-radius:10px;background:rgba(255,255,255,.48);border:1px solid rgba(244,143,177,.12);font-size:12px;line-height:1.55;color:var(--mp-txt);}
...
.mp-post-ct{font-size:13px;line-height:1.55;margin-bottom:8px;white-space:pre-wrap;word-break:break-word;}
...
.mp-liked-by{margin-top:6px;font-size:11px;line-height:1.45;color:var(--mp-txt-l);background:rgba(255,255,255,.3);border-radius:9px;padding:6px 8px;white-space:pre-wrap;word-break:break-word;}
.mp-social-head-actions{margin-left:auto;display:flex;align-items:center;gap:6px;}
.mp-pill-btn{background:linear-gradient(135deg,#f48fb1,#e91e63);color:#fff;border:none;border-radius:16px;padding:5px 12px;font-size:11px;font-weight:700;cursor:pointer;font-family:var(--mp-font);box-shadow:0 5px 12px rgba(233,30,99,.18);white-space:nowrap;}
.mp-pill-btn-ghost{background:rgba(255,255,255,.72);color:var(--mp-pink-dk);border:1px solid rgba(244,143,177,.24);box-shadow:none;}
...
.mp-comments.scroll::-webkit-scrollbar-thumb{background:rgba(244,143,177,.35);border-radius:999px;}
.mp-comment{font-size:11px;line-height:1.45;color:var(--mp-txt-l);background:rgba(255,255,255,.32);border-radius:9px;padding:5px 7px;white-space:pre-wrap;word-break:break-word;}
.mp-comment.clickable{cursor:pointer;}
.mp-comment.clickable:hover{background:rgba(255,255,255,.46);}
.mp-comment.reply{margin-left:16px;border-left:2px solid rgba(244,143,177,.25);border-top-left-radius:4px;border-bottom-left-radius:4px;}
.mp-comment.empty{color:var(--mp-txt-l);font-style:italic;background:rgba(255,255,255,.2);}
...
.mp-sec-toggle-tag{font-size:10px;font-weight:700;color:var(--mp-pink-dk);padding:2px 8px;border-radius:999px;background:rgba(244,143,177,.12);border:1px solid rgba(244,143,177,.2);}
...
.mp-ibtn{background:rgba(244,143,177,.12);border:1px solid rgba(244,143,177,.18);border-radius:8px;min-height:32px;padding:0 12px;font-size:11px;cursor:pointer;color:var(--mp-pink-dk);font-weight:600;font-family:var(--mp-font);display:inline-flex;align-items:center;justify-content:center;}
.mp-ibtn-view{background:rgba(144,202,249,.16);border-color:rgba(144,202,249,.35);color:#1565c0;}
...
.mp-lorebook-content{font-size:12px;line-height:1.6;white-space:pre-wrap;max-height:260px;overflow-y:auto;padding:10px;border:1px solid rgba(244,143,177,.18);border-radius:8px;background:rgba(255,255,255,.55);}
