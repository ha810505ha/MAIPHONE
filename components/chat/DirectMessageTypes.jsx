...
export function OnlineChatMessage({ message, isUser, active, highlighted, displayContent, innerThought, thinking, voiceAction, voicePlayback, onToggle, onEdit }) {
  ...
  return <div data-message-id={message.id} className={`mp-msg-wrap ${isUser ? "mp-msg-wrap-user" : "mp-msg-wrap-ai"} ${intercepted ? "mp-msg-wrap-intercepted" : ""} ${highlighted ? "mp-thought-jump-highlight" : ""}`}>{isUser && <button className={`mp-msg-editbtn ${active ? "" : "mp-msg-editbtn-hidden"}`} onClick={onEdit}>✎</button>}<div className={`mp-thought-stack ${isUser ? "mp-thought-stack-user" : ""}`}><div className={`mp-msg-delivery-row ${isUser ? "is-user" : ""}`}>{isUser && intercepted && <span className="mp-msg-intercepted-mark" title={interceptedLabel}>!</span>}<div className={`mp-msg ${isUser ? "mp-msg-user" : "mp-msg-ai"}`} onClick={onToggle}>{message.image && <img src={`data:image/png;base64,${message.image}`} className="mp-msg-img" alt="" />}{message.pseudoImage && <PseudoImageBubble pseudoImage={message.pseudoImage} />}{message.pseudoVoice && <PseudoVoiceBubble pseudoVoice={message.pseudoVoice} playback={voicePlayback} />}{displayContent && !message.pseudoVoice && <div>{displayContent}</div>}<div className="mp-msg-t">{new Date(message.time).toLocaleTimeString("zh-TW", { hour: "2-digit", minute: "2-digit" })}{!isUser && message.ttsGeneratedAt && <span className="mp-msg-tts-mark" title="曾生成語音" aria-label="曾生成語音">🔊</span>}</div></div>{!isUser && intercepted && <span className="mp-msg-intercepted-mark" title={interceptedLabel}>!</span>}</div>{intercepted && <div className="mp-msg-intercepted-label">{interceptedLabel}</div>}{!isUser && innerThought}{!isUser && thinking}</div>{!isUser && <button className={`mp-msg-editbtn ${active ? "" : "mp-msg-editbtn-hidden"}`} onClick={onEdit}>✎</button>}{!isUser && voiceAction}</div>;
}
