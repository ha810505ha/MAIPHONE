...
export default function GroupChatContent({ messages, isTyping, activeMessageId, setActiveMessageId, playerAvatar, chatMsgsRef, messagesEndRef, onScroll, isConnectionErrorNotice, onRetry, onEdit, onDelete, showScrollToBottom, onScrollToBottom, chatImage, onClearImage, actionPanelOpen, setActionPanelOpen, fileInputRef, onImageUpload, chatInput, setChatInput, onSend, tr }) {
  return (
    <>
      <div className="mp-cr" style={{ flex: 1, minHeight: 0 }}>
        <div className="mp-msgs" ref={chatMsgsRef} style={{ flex: 1, minHeight: 0, paddingBottom: 12 }} onScroll={(event) => onScroll(event.currentTarget)}>
          {messages.map((message) => message.role === "system_notice" ? (
            <div key={message.id} className="mp-msg-note-wrap"><div className="mp-msg-note"><div>{message.content}</div>{isConnectionErrorNotice(message.content) && <button className="mp-retry-btn" disabled={isTyping} onClick={(event) => { event.stopPropagation(); onRetry(message.id); }}>{tr("重新生成", "Regenerate", "再生成", "다시 생성")}</button>}</div></div>
          ) : (
            <div key={message.id} className={`mp-msg-wrap ${message.role === "user" ? "mp-msg-wrap-user mp-group-msg-wrap-user" : "mp-msg-wrap-ai mp-group-msg-wrap-ai"}`}>
              <div className="mp-group-msg-meta"><div className="mp-group-msg-avatar">{message.role === "user" ? (playerAvatar ? <img src={playerAvatar} alt="" /> : null) : (message.speakerAvatar ? <img src={message.speakerAvatar} alt="" /> : "👥")}</div>{message.role !== "user" && <div className="mp-group-msg-name">{message.speakerName || tr("群組", "Group", "グループ", "그룹")}</div>}</div>
              <div className={`mp-msg ${message.role === "user" ? "mp-msg-user" : "mp-msg-ai"}`} onClick={() => setActiveMessageId((previous) => previous === message.id ? null : message.id)}>{message.image && <img src={`data:image/png;base64,${message.image}`} className="mp-msg-img" alt="" />}{message.content && <div>{message.content}</div>}<div className="mp-msg-t">{new Date(message.time).toLocaleTimeString("zh-TW", { hour: "2-digit", minute: "2-digit" })}</div></div>
      ...
      {actionPanelOpen && <div className="mp-chat-actions"><button className="mp-chat-action" onClick={() => { setActionPanelOpen(false); fileInputRef.current?.click(); }}><span className="mp-chat-action-i">🖼</span><span>{tr("相片", "Photo", "写真", "사진")}</span></button><button className="mp-chat-action" disabled><span className="mp-chat-action-i">📅</span><span>{tr("日程", "Schedule", "予定", "일정")}</span></button><button className="mp-chat-action" disabled><span className="mp-chat-action-i">⚙️</span><span>{tr("更多", "More", "その他", "더보기")}</span></button></div>}
      <div className="mp-inp-bar" style={{ borderTopLeftRadius: 0, borderTopRightRadius: 0 }}><button className={`mp-btn mp-btn-img ${actionPanelOpen ? "active" : ""}`} onClick={() => setActionPanelOpen((value) => !value)}>＋</button><input type="file" ref={fileInputRef} accept="image/*" style={{ display: "none" }} onChange={onImageUpload} /><div className="mp-inp-wrap"><textarea className="mp-inp" placeholder={tr("輸入群組訊息...", "Type a group message...", "グループメッセージを入力...", "그룹 메시지를 입력...")} rows={1} maxLength={4000} autoComplete="off" autoCorrect="off" autoCapitalize="sentences" spellCheck={false} data-form-type="other" data-lpignore="true" value={chatInput} onChange={(event) => setChatInput(event.target.value.slice(0, 4000))} /><div className="mp-char-counter">{chatInput.length}/4000</div></div><button className="mp-btn mp-btn-send" onClick={onSend}>➤</button></div>
    </>
  );
}
