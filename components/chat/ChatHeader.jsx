  ...
  return (
    <>
      <div className="mp-hdr">
        ...
        {hasRooms ? <button type="button" className="mp-htitle" onClick={() => setRoomPickerOpen(true)} style={{ flex: 1, minWidth: 0, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap",border:0,background:"transparent",color:"inherit",padding:0,textAlign:"left",cursor:"pointer" }}>{item.name} <span style={{fontSize:10,color:"var(--mp-txt-l)"}}>▾</span></button> : <div className="mp-htitle" style={{ flex: 1, minWidth: 0, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{item.name}</div>}
